...
_metrics_dir=/mnt/lotus/zhangzhichaoHome/tmp/metrics/\$_one_day_from_now

# Find and prune any stale metrics directories.
find /mnt/lotus/zhangzhichaoHome/tmp/metrics/ -maxdepth 1 -mindepth 1 -type d -printf "%f\n" \
    | xargs -I {} bash -c 'if (({} < \$(date +%s))) ; then rm -rf /mnt/lotus/zhangzhichaoHome/tmp/metrics/{} ; fi' 2> /dev/null
...
./fil-proofs-tooling/scripts/retry.sh 42 10 60000 \
    ./fil-proofs-tooling/scripts/with-lock.sh 42 /mnt/lotus/zhangzhichaoHome/tmp/metrics.lock \
